...
Confidential meeting data lives OUTSIDE the repo at
`~/Library/Application Support/MeetFlow` (gitignored, Time-Machine only, never synced).

Day-to-day, the only action is **Ctrl+Alt+M** (start/stop); everything lands in one fixed dir; NO client
...
**Phase 1 — DONE and proven on a real 16-min call:**
...
`~/dotfiles/hammerspoon` — it only writes a one-word command to
...
`~/dotfiles/launchd/Library/LaunchAgents/com.ververflow.meetflow.plist` mirrors the whisper-server
...
- **Stable signing — DONE (part of 4b, landed early):** the `.app` is signed with a STABLE
  ...
  Helpers: `swift-sidecar/grant-perm.sh` (grant), `swift-sidecar/verify-tap.sh` (verify); `swift-sidecar/build.sh`.
...
```
Ctrl+Alt+M (Hammerspoon meetflow.lua, ~/dotfiles)
...
- `client_slug` data hygiene: an untagged meeting can end up with a slug derived from the time
  (e.g. an untagged call shows `client_slug = "110000"`). Cosmetic; revisit with `meetflow tag`.
...
1. **Fix the AEC sample-rate bug** so `aec = "on"` works → clean separation WITHOUT headphones
   (common when recording on speakers). The tap "them" channel is written 48 kHz but read as 16 kHz in the

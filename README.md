...
- Trigger + menubar: Hammerspoon (`~/dotfiles/hammerspoon/.hammerspoon/meetflow.lua`)
- Autostart: launchd (`~/dotfiles/launchd/.../com.ververflow.meetflow.plist`)
...
### How it's wired (machine-as-code, in `~/dotfiles`)

```sh
# Hammerspoon trigger (Ctrl+Alt+M + menubar):
stow -d ~/dotfiles hammerspoon && hs -c "hs.reload()"

# launchd daemon (autostart at login, supervised):
stow -d ~/dotfiles launchd
